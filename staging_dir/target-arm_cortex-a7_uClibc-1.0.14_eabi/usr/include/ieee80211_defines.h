...
#define IEEE80211_RX_FIRST_MSDU     0x01
#define IEEE80211_RX_LAST_MSDU      0x02
#define IEEE80211_RX_DECRYPTED      0x04
/* first_msdu=1, last_msdu=1  ==> non-Aggregation frame*/
/* first_msdu=1, last_msdu=0  ==> first msdu*/
/* first_msdu=0, last_msdu=0  ==> msdu frame between first and last msdu*/
/* first_msdu=0, last_msdu=1  ==> last msdu*/
...
typedef struct _wlan_event_handler_table {
    ...
    void (*wlan_bringdown_ap_vaps) (os_if_t osif);     /* Bringidown ap vaps */
...
#define LAST_FOUR_ACK_RSSI 4
#define LAST_FOUR_ACK_AGE 4

typedef struct _rssi_snapshot {
    u_int8_t rssi[LAST_FOUR_ACK_RSSI];
    u_int8_t time_s[LAST_FOUR_ACK_AGE];
    u_int16_t count;
} rssi_snapshot_t;
...
struct ieee80211_nodestats {
    ...
    u_int64_t    ns_ppdu_tx_rate;        /* Avg per ppdu tx rate in kbps */
    u_int64_t    ns_ppdu_rx_rate;        /* Avg per ppdu rx rate in kbps */
    rssi_snapshot_t rssi_ack;              /* Last four ACK RSSI */
    rssi_snapshot_t rssi_bcn;              /* last four beacon RSSI */
};
...
typedef enum _ieee80211_param {
...
#endif
#if ATH_SSID_STEERING
    IEEE80211_VAP_SSID_CONFIG,               /* VAP Public or Private */
#endif
    ...
    IEEE80211_CONFIG_EXT_NSS_CAPABLE,         /* Enbale/Disable EXT NSS Capability */
    IEEE80211_CONFIG_CSL_SUPPORT,             /* Configure Connection state logging */
    IEEE80211_CONFIG_TX_CAPTURE_DA,           /* Enable Tx capture for DA radios */
    ...
    IEEE80211_GET_VAP_ACTIVITY,               /* % of time the vap couldnot tx or rx pkts to/fro clients */
    IEEE80211_PRB_RATE,                       /* ieee rate to be used for probe-response */
    IEEE80211_OCE_HLP,                        /* Enable OCE FILS HLP */
    IEEE80211_STEALTH_DOWN,                   /* Enable stealth mode interface down without explicitly disconnecting STAs */
...
static inline char * msg_type_to_str (int msg_type) {
...
#define ACS_RANK_DESC_DBG_LEN 48
