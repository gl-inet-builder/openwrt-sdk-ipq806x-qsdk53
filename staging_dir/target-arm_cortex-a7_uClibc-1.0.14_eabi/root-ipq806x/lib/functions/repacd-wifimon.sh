...
ping_running=0 last_ping_gw_ip= gw_lost_count=0
    ...
    if [ "$associated" -gt 0 -o "$resolving" -gt 0 ]; then
        ...
        if [ "$daisy_chain" -gt 0 -a "$force_down_5g" -eq 0 ]; then
            ...
            if [ "$rate_num" -le "$rate_samples" ] && \
                ...
                # more than 30 seconds of consecutive ping lost to gw, restart network to aquire new ip.
                if [ "$gw_lost_count" -ge 5 ]; then
                    gw_lost_count=0
...
# Set the Root AP distance based on STA association state.
# input $1: config: interface configuration name
# input $2: root_distance_24g: 2.4G link root AP distance
# input $3: root_distance_5g: 5G link root AP distance
# output $4: root_distance: final configured root distance
__repacd_wifimon_set_root_distance() {
    ...
    if [ "$mode" = "ap" ]; then
        if [ -n "$sta_iface_24g" -a -n "$sta_iface_5g" ]; then
            root_distance_24g=$2
            root_distance_5g=$3
        ...
        if [ "$root_distance_24g" -le "$root_distance_5g" ]; then
            ...
            eval "$4=$root_distance_24g"
        else
            iwpriv $iface set_whc_dist $root_distance_5g
            eval "$4=$root_distance_5g"
        fi
    fi
}
...
repacd_wifimon_check() {
    if [ -n "$sta_iface_5g" ]; then
        local new_root_distance root_distance_24g=0 root_distance_5g=0
        ...
        config_foreach __repacd_wifimon_set_root_distance wifi-iface \
            $root_distance_24g $root_distance_5g new_root_distance
