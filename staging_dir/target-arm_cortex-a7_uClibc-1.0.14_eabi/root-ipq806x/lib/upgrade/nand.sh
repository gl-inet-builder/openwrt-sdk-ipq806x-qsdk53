...
check_is_openwrt1907_firmware() {
	case "$1" in
	gl-ar750s)
		firmware_name="sysupgrade-glinet_gl-ar750s-nor-nand"
		;;
	esac
	control_length=`(tar xf $2 $firmware_name/CONTROL -O | wc -c) 2> /dev/null`
	echo $control_length
}
...
nand_do_platform_check() {
	...
	[ "$control_length" = 0 ] && {
		control_length=$(echo $(check_is_openwrt1907_firmware $board_name $tar_file))
	}
	local file_type="$(identify $2)"

	[ "$control_length" = 0 -a "$file_type" != "ubi" -a "$file_type" != "ubifs" ] && {
		echo "Invalid sysupgrade file."
		echo "length $control_length file_type $file_type."
		return 1
	}
