...
EOF
	[ -z "$proto" -a "$key_mgmt" != "NONE" ] || {\
		...
		wpa_cli -g /var/run/wpa_supplicantglobal interface_add  $ifname /var/run/wpa_supplicant-$ifname.conf athr /var/run/wpa_supplicant-$ifname "" $bridge >/dev/null
...
wpa_supplicant_run() {
	...
	wpa_cli -g ${_rpath}global interface_add  $ifname ${_config} nl80211 ${_rpath}-$ifname "" >/dev/null
