		...
		[ -f /sys/class/net/${dev}/parent ] && { \
			...
			[ -n "$parent" -a "$parent" = "$device" ] && { \
				[ -f "/var/run/hostapd-${dev}.lock" ] && { \
					wpa_cli -g /var/run/hostapd/global raw REMOVE ${dev} >/dev/null
					rm /var/run/hostapd-${dev}.lock
				}
				[ -f "/var/run/wpa_supplicant-${dev}.lock" ] && { \
					wpa_cli -g /var/run/wpa_supplicantglobal  interface_remove  ${dev} >/dev/null
...
enable_qcawifi() {
	...
	#Default disable some log
	iwpriv "$phy" dl_reporten 0
	iwpriv "$phy" smartLogEn 0
	iwpriv "$phy" ps_report 0
	...
	[ "$device" = wifi0 ] && iwpriv "$phy" set_fw_recovery 1
	...
	for vif in $vifs; do
		...
		ifname_uci=$(uci -q get wireless.$vif.ifname)
		[ -n "$ifname_uci" ] && ifname=$ifname_uci
		[ -n "${DEBUG}" ] && echo wlanconfig "$ifname" create wlandev "$phy" wlanmode "$wlanmode" ${wlanaddr:+wlanaddr "$wlanaddr"} ${nosbeacon:+nosbeacon}
		if [ -z "$recover" ] || [ "$recover" -eq "0" ]; then
			ifname=$(/usr/sbin/wlanconfig "$ifname" create wlandev "$phy" wlanmode "$wlanmode" ${wlanaddr:+wlanaddr "$wlanaddr"} ${bssid:+-bssid "$bssid"} ${nosbeacon:+nosbeacon})
			[ $? -ne 0 ] && {
		...
		if [ -f /tmp/pairing_in_process ]; then
