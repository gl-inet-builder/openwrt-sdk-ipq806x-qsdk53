...
proto_3g_setup() {
	...
	if [ "$ipv6_enabled" = "1" -a "$ipv6_wan_interface" = "modem_$modem_bus" ];then
		sed -i "/IPV4V6/d" /etc/chatscripts/evdo.chat 2>/dev/null
		sed -i "s/ATZ/ATZ\n\"\"      'AT+CGDCONT=1,\"IPV4V6\"'/g" /etc/chatscripts/evdo.chat 2>/dev/null
		sed -i "s/IP.*\",/IPV4V6\",/g" /etc/chatscripts/3g.chat 2>/dev/null
	else
		sed -i "/IPV4V6/d" /etc/chatscripts/evdo.chat 2>/dev/null
		sed -i "s/IPV4V6.*\",/IP\",/g" /etc/chatscripts/3g.chat 2>/dev/null
	fi
